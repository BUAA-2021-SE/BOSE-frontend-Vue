    ...
    <div v-if="sharedState.is_authenticated" class="mx-auto" width="80vw">
      <v-row>
        ...
        <v-col md="7" class="my-auto">
          ...
          <v-card-content>
          <img v-if="editForm.cover" :src="editForm.cover" :style="{'margin-left':'20px','border-radius':'20px','max-width':'250px','min-height':'100%','max-height':'150px'}">
          <div v-else>
          <br><br><br><br><br><br>
          </div>
          </v-card-content>
      ...
      <div style="z-index:-10">
        <mavon-editor ref="md" v-model="editForm.body" :toolbars="tools" @imgAdd="imgAdd"
  ...
  methods: {
    ...
    onSubmitAdd() {
      ...
      Post.editBlog(this.$route.params.id, formData)
          ...
          .catch((err) => {
            console.error(err);
            this.$toasted.error('不知道哪里出了什么问题捏。', {icon: 'check'});
          })
    },
    onCommitDraft() {
