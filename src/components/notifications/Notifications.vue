<template>
  ...
  <v-col cols="12" md="1"></v-col>
    ...
    <v-card-text>{{user.about_me}}</v-card-text>
        ...
        ReceivedComments
        <span class="red--text ms-4" v-if="unread_comments!=0">new</span>
        </v-btn>
        <v-btn :to="{name: 'ReceivedMessages'}" 
        block color="blue" outlined>
        <v-icon color="green">mdi-email</v-icon>
        ReceivedMessages
        <span class="red--text ms-4" v-if="unread_messages!=0">new</span>
        </v-btn>
        <v-btn :to="{name: 'Likes'}" 
        block color="blue" outlined>
        <v-icon color="green">mdi-heart</v-icon>
        Likes
        <span class="red--text ms-4" v-if="unread_likes!=0">new</span>
        </v-btn>
        <v-btn :to="{name: 'FollowingPosts'}" 
        block color="blue" outlined>
        <v-icon color="green">mdi-access-point</v-icon>
        Followings
        <span class="red--text ms-4" v-if="unread_followings!=0">new</span>
        </v-btn>
    </v-card>
    </v-col>
    <v-col cols="12" md="6">
